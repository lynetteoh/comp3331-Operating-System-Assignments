...
//use mutual exclusion lock so that only one thread can perform operations in the critical region at a time
struct lock *mutex;
...
static void adder(void * unusedpointer, unsigned long addernumber)
{
        ...
        while (flag) {
                ...
                // accquire mutual exclusion lock and enter critical region
                lock_acquire(mutex); 
                ...
                if (a < NADDS) {
                        ...
                        // release mutual exclusion lock and exit critical region
                        lock_release(mutex);
...
int maths (int data1, char **data2)
{
        ...
        // panic if semaphore failed to create
        if (finished == NULL) {
                panic("maths: sem create failed");
        }

        // create a mutual exclusion lock for thread
        mutex = lock_create("mutex");

        // panic if mutual exclusion lock failed to create
        if (mutex == NULL) {
                panic("maths: mutual exclusive lock create failed");
        }
